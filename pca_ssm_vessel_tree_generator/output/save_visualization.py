...
def save_tree_visualization(
    ...
    for b_idx in range(m_branches):
        ...
        name = branch_names[b_idx] if branch_names and b_idx < len(branch_names) else f"branch_{b_idx}"
        if name in colors:
            lw = float(np.mean(radii) * 2.0)
            ax.plot(pts[:, 0], pts[:, 1], pts[:, 2], label=name, color=colors[name], linewidth=lw)
        else:
            lw = float(np.mean(radii) * 2.0)
...
def save_pipeline_qc_report(output_dir: Path, pipeline_report: dict[str, Any]) -> bool:
    ...
    stages = ["Source audited", "Assignments resolved", "Core anatomy pass", "PCA eligible"]
    counts = [
    ...
    if k_modes:
        axes[0, 1].plot(range(1, k_modes + 1), np.linspace(0, cum_var, k_modes), "o-", color="purple")
    ...
    phases = np.linspace(0.0, 0.9, n_phases) if n_phases else np.asarray([])
    radial = float(c_mot.get("motion_parameters", {}).get("radial_amplitude", 0.0))
    volumes = 100.0 * (1.0 - radial * np.sin(np.pi * phases / 0.7))
