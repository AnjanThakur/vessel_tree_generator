...
def main():
    ...
    # Resolve BASE directory (lca_vessel_tree_generator) relative to this file
    BASE = Path(__file__).resolve().parent.parent
    
    parser = argparse.ArgumentParser(description="LCA Patient Centerline Preprocessor")
    parser.add_argument("--dataset-root", type=str, default=r"D:\Projects\lca_vessel\LCA_dataset",
                        help="Path to raw patient dataset directory (external input)")
    parser.add_argument("--output-dir", type=str, default=str(BASE / "processed_dataset"),
                        help="Output directory for normalized patient centerlines")
    parser.add_argument("--ctrl-points-dir", type=str, default=str(BASE / "LCA_branch_control_points" / "generated"),
                        help="Output directory for collective control points databases")
    args = parser.parse_args()
    
    dataset_root = args.dataset_root
    output_base_dir = args.output_dir
    ctrl_points_dir = args.ctrl_points_dir
    ...
    if collective_lmca:
        ...
        # --- NEW CODE BEGINS: Unified Tree Statistics Calculation ---
        # Strictly concatenate branches along axis 1 (points axis) in fixed topological order:
        # 1. LMCA (5 points, indices 0-4)
        # 2. LAD  (12 points, indices 5-16)
        # 3. LCX  (10 points, indices 17-26)
        # Total points = 27
        # Array Shape: (number_of_patients, 27, 3)
        all_tree_ctrl_points = np.concatenate([lmca_stacked, lad_stacked, lcx_stacked], axis=1)

        # Compute coordinate-wise mean and standard deviation across patient population axis (axis 0)
        tree_mean = np.mean(all_tree_ctrl_points, axis=0)
        tree_std = np.std(all_tree_ctrl_points, axis=0)

        # Save as permanent preprocessing outputs
        ...
        print("Successfully exported collective patient LCA tree control points, mean, and standard deviation.")
        # --- NEW CODE ENDS ---
    else:
