# Legacy Synthetic Coronary Tree Generator — Technical Design Document

> Historical design reference. It predates the final data audit and describes
> an intended LCA + RCA model. The submitted, validated statistical scope is
> LCA-only (LMCA/LAD/LCX); see `README.md` and `SUBMISSION_GUIDE.md` for the
> implemented contract. Do not cite this document as completed RCA capability.
